            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="flex leading-none text-indigo-600 divide-x divide-indigo-400">
                        ...
                        <li class="px-4 text-gray-700" aria-current="page">Daftar Ulang</li>
                    </ol>
                </nav>
            </div>
            <div class="text-right">
              <Link :href="route('registrations.create')"><jet-button class="bg-blue-500 hover:bg-indigo-400 text-white font-bold py-2 px-4 rounded">Bayar Daftar Ulang</jet-button></Link>
            </div>
        </div>
        </template>
                            ...
                            <div class="container flex px-2 py-2">              
                                <div class="relative">
                                    ...
                                    <div class="absolute top-2 right-3"> 
                                        <Link :href="route('registrations.index', { 'search': seachKey } )">
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="(item, index) in payments.data" :key="item.id">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <Link :href="route('registrations.edit', item.id)" type="button"><jet-button class="bg-coolGray-500 ml-4 hover:bg-coolGray-400">Edit</jet-button></Link>
                                            &nbsp;<jet-danger-button @click="confirmRegDeletion(item.id)">Hapus </jet-danger-button></td>
                                    </tr>
                                </tbody>
                            </table>
...
<script>
    ...
    export default defineComponent({
        ...
        methods:{
            quickSearch(){                
                this.$inertia.get(route('registrations.index', { 'search': this.seachKey } ),{})
            },
            clearSearchBar(){
                if(route().params.search){
                    this.$inertia.get(route('registrations.index'),{})
                }else{
                    this.seachKey = null
                }
            },
            confirmRegDeletion(id) {
                this.itemId = id;
                this.confirmingUserDeletion = true;
                if (!confirm('Are you sure want to remove?')) return;
                this.form.delete(route('registrations.destroy', this.itemId), {

            ...
            <div>
                <nav aria-label="breadcrumb">
                    <ol class="flex leading-none text-indigo-600 divide-x divide-indigo-400">
                        ...
                        <li class="px-4 text-gray-700" aria-current="page">Angkatan</li>
                    </ol>
                </nav>
            </div>
            <div class="text-right">
              <Link :href="route('rooms.create')"><jet-button class="bg-blue-500 hover:bg-indigo-400 text-white font-bold py-2 px-4 rounded">Tambah Angkatan</jet-button></Link>
            </div>
        </div>
        </template>
                        ...
                        <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
                            <div class="container flex px-2 py-2">              
                                <div class="relative">
                                    ...
                                    <div class="absolute top-2 right-3"> 
                                        <Link :href="route('rooms.index', { 'search': seachKey } )">
                            ...
                            <table class="min-w-full divide-y divide-gray-200 w-full">
                                ...
                                <tr>
                                    ...
                                    <th scope="col" class="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">SPP</th>
                                    <th scope="col" class="px-6 py-3 bg-gray-50 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Daftar Ulang</th>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-for="(item, index) in rooms.data" :key="item.id">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ item.priceRupiah }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ item.reRegistrationRupiah }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ item.formattedCreatedAt }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                            {{ item.formattedUpdatedAt }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <Link :href="route('rooms.edit', item.id)" type="button"><jet-button class="bg-coolGray-500 ml-4 hover:bg-coolGray-400">Edit</jet-button></Link>
                                            &nbsp;<jet-danger-button @click="confirmUserDeletion(item.id)">Hapus </jet-danger-button></td>
                                    </tr>
                                </tbody>
                            </table>
                            <pagination :links="rooms.links"></pagination> 
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default defineComponent({
        ...
        methods:{
            quickSearch(){                
                this.$inertia.get(route('rooms.index', { 'search': this.seachKey } ),{})
            },
            clearSearchBar(){
                if(route().params.search){
                    this.$inertia.get(route('rooms.index'),{})
                }else{
                    this.seachKey = null
                }
            },
            ...
            deleteitem() {
                ...
                // this.form.delete(route('rooms.destroy', this.itemId), {
